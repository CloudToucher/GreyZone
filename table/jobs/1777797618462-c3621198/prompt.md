...
- player_seat: 习风
- character_output_path: characters/active/影子.md
- result_raw_path: table/jobs/1777797618462-c3621198/result.raw.md

## 必须写入角色卡
使用 Write 工具写入 `characters/active/影子.md`。frontmatter 必须包含 name, controller, level, xp, blood, energy, attributes, location, sceneId, partyId, visibilityScope。
...
## 返回标签协议（必须使用）
你必须把完整原始返回写入 `table/jobs/1777797618462-c3621198/result.raw.md`。WebUI 会保存 raw，再解析标签。
...
## 文件输出硬要求
你必须使用 Write 工具将完整结果写入 `table/jobs/1777797618462-c3621198/result.raw.md`。
